...
namespace PG
{
    public partial class GestureForm : Form
    {
        ...
        private void comboPointing_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                labelA.Visible = true;  //boundary A
                labelB.Visible = true;
                labelD.Visible = true;
                textA.Visible = true;   //value of boundary A=500
                textB.Visible = true;
                textD.Visible = true;
            }
            else
        ...
        private void GestureForm_Load(object sender, EventArgs e)
        {
            ...
            string[] lines = System.IO.File.ReadAllLines(@"Resources\comboTrain.txt");
            numOfGestures = lines.Length;                                  //change
            Console.WriteLine("num of gestures" + numOfGestures);
            ...
            labelToName = new Dictionary<int, String>();   //data dumping for simulator
            //string[] lines = System.IO.File.ReadAllLines(@"Resources\comboTrain.txt");
            int cCount = lines.Length;
            ...
            labelToName.Add(5, "Stop");
            labelToName.Add(6, "Pick Up");
            labelToName.Add(7, "Repeated Pick Up");
            labelToName.Add(8, "Free Movement");
            labelToName.Add(9, "Follow");
            labelToName.Add(10, "Unfollow");
            labelToName.Add(11, "Go Behind");
            */
            for(int i = 0; i < cCount; i++)
            {
                labelToName.Add(i, lines[i]);
            }
            dictionaryCount = labelToName.Count;
            //Console.WriteLine(dictionaryCount);
        ...
        private void Reader_FrameArrived(object sender, BodyFrameArrivedEventArgs e)
        {
            ...
            {
                fpscounter++;
                if (fpscounter % 3 == 0) return;  //frame 20
            }
                ...
                if (dataReceived)
                {
                    ...
                    if (body != null && this.bodyTracked && body.IsTracked)
                    {
                        ...
                        if (isTracking == false && Math.Abs(shoulderRightY - elbowRightY) <= 3.0 && Math.Abs(shoulderRightY - handRightY) < 8.0)
                        {
                            ...
                            if (comboPointing.SelectedIndex == 0)             //method 1 robot movement drawing
                            {
                            ...
                            else                //method 2 robot movement drawing
                            {
                                start = new Point((double)shoulderRight.X * 100.0, (double)shoulderRight.Z * 100.0);
                                end = new Point((double)handRight.X * 100.0, (double)handRight.Z * 100.0);
                                ...
                                if (distance(intersection, robot) > 10.0)            //robot moving towards intersection point
                                {
                                    length = speed;
                                ...
                                else            //robot is now on intersection point
                                {
                                    if (goForward)
                                    {
                                        length = speed;
                        ...
                        {
                            labelGesture.Text = "Performing Gesture";    //hand closed & gesture tracking 
                            fpscounter = 0;
                            handX.Clear();
                            handY.Clear();
                            handZ.Clear();
                        ...
                        if (isTracking == true && body.HandLeftState == HandState.Open)
                        {
                            isTracking = false;
                            Console.WriteLine();
                            if (isTraining == true)      //ratul        //training a gesture
                            {
                                // comboTrain.SelectedIndex change
                                int labelNumber;
                                labelNumber = comboTrain.SelectedIndex;
                                // labelNumber = numOfGesture + 1; ratul
                                // numOfGesture +=1;  ratul
                                featureVector.Clear();
                                ...
                                trainingLabels.Add(labelNumber);     //adding class label

                                labelGesture.Text = "Gesture Recognition: Idle";
                            }
                            else            //recognition mode on
                            {
                                featureVector.Clear();
                                for (int i = 0; i < handX.Count; i++) featureVector.Add(handX[i]);
                                ...
                                for (int i = 0; i < numOfGestures; i++)
                                {
                                    ...
                                    Console.WriteLine(likelihood + " " + recognizedLabel);
                                    if (likelihood >= -100.0 && likelihood > mx)
                                    {
                                        mx = likelihood;
                                        recognizedLabel = i;
                                        Console.WriteLine(mx + " " + recognizedLabel);
                                    }
                                }
                                ...
                                {
                                    labelGesture.Text = "Gesture Performed: Unrecognized Gesture\n";
                                    //System.Windows.Forms.MessageBox.Show("My message here");
                                    string message = "Do you want to add your new gesture?";
                                    string caption = "Unrezognized Gesture";
                                    ...
                                    {
                                        String gg = @"Resources\gg.txt";
                                        //Console.WriteLine(gg);
                                        for (int i = 0; i < featureVector.Count; i++)
                                        {
                                            System.IO.File.AppendAllText(gg, featureVector[i].ToString());
                                        }
        ...
        {
            labelMode.Text = "Mode: Adding New Gesture";
            comboTrain.Visible = true;
            isTraining = false;
            addNew = true;
            string strName = Microsoft.VisualBasic.Interaction.InputBox("Write the name of your new gesture","New Gesture Label","",0,0);
            Console.WriteLine (strName);

            dictionaryCount += 1;
            numOfGestures += 1;
            labelToName.Add(dictionaryCount, strName);

            //Console.WriteLine(dictionaryCount);
            File.AppendAllText(@"Resources\comboTrain.txt", strName + Environment.NewLine);
            
            this.comboTrain.Items.AddRange(new object[] { strName });

            //Console.WriteLine("n o g " + numOfGestures);
        }
